...
Next model is the support vector machine. We will first use the Radial, then the poly model.
...
# plot(model_svm, german.tr, RESPONSE ~ as.line(USED_CAR), 
#      slice = list(DURATION = 3, CHK_ACCT = 6))
```

As we can see from the model summary and later confirmed using the best Tune and the plot, the model is well fitted to the data when sigma = 0.001 and the cost C = 100. As we can see, the cost is very large. Having a large cost mean that the margins are going to be very small, and the border is not smooth. The risk of having such a large Cost is that the model is over-fitted. If that is the case, we wouldn't be able to well predict the outcome variable on the test set. We can look at the confusion matrix and see what happens.
...
The model's Accuracy is of 82%, which is the highest seen so far. However, when looking at the Sensitivity and the Specificity, we realize that it is able to predict almost perfectly when RESPONSE is 1, but pretty badly when the outcome is 0. Indeed, the Specificity is of 94% which is a really good value, but the Sensitivity is of only 60% showing a poor ability in prediciting the bad creditors. 
In the light of what we have seen, we can confirm that the model is not overfitted. If that was the case it wouldn't be able to make good predictions and the Accuracy would be definetly lower. 
...
plot(model_svm_poly)
```
This support vector machine model doesn't only tune the Cost parameter, but also the scale and the degree. The output of the model's summary, as well as the bestTune function and the graph show that the model gives the best results when degree = 1, scale = 1, and C = 100. We are now going to analyse the confusion matrix and see if the model's prediction capacity are good.
...
confusionMatrix(predict.train(model_svm_poly, newdata = german.te),
                german.te$RESPONSE)
```
Like in the previous support vector machine model, this one is better at predicting the good creditors ("1") compared to the bad ones. Indeed, even if the Accuracy is at 70% which is a good value, the Sensitivity is extremely low (only 39%). In fact the confusion matrix shows that the model is able to predict only 32 times out of 82 the bad creditors. 
Hence, we would prefer not using this model as it wouldn't be bale to make the correct predictions. 
...
model_rf
# question --> mtry = 34 but we have less than 34 variables: does it make sense?
