...
The model's Accuracy is of 90.4%, which is the highest seen so far. However, when looking at the Sensitivity and the Specificity, we realise that it is able to predict almost perfectly when RESPONSE is 0, and quite well when the outcome is 1. Indeed, the Specificity is of 85% which is a good value, but the Sensitivity is of 100%, which means that the model can perfectly predict bad creditors.

In the light of what we have seen, we can confirm that the model is not under-fitted. If that was the case it would not be able to make good predictions and the Accuracy would be definetly lower. 
...
ctrl<-trainControl(method="cv", number=3, sampling = "down")

set.seed(123)
system.time(model_svm_poly<-train(RESPONSE~., data=german.tr, method="svmPoly", trControl=ctrl, 
                            tuneGrid=gr.poly))
...
Unlike in the previous Support Vector Machine model, this model has a much lower Accuracy (66.8%). However, we can highlight that, similarly to the previous model, this one is slightly better at predicting the bad creditors ("0") compared to the good ones. Indeed, the model is able to predict correctly bad creditors 56 times out of 82 and good creditors 111 times out of 168.

Hence, we would prefer not using this model since other models are much more performant. 
...
To put in a nutshell, we have observed that overall most of the models are predicting quite well both classes. When considering the top 3 in terms of overall Accuracy, the ones that stand out are the Radial SVM model (90.4%), followed by the QDA model (71.2%) and Random Forest model (69.6%). 

However, depending on the aim of the project, the choice of model may differ. To further explain, if the goal is to predict good credit candidates, one should consider looking at the models having the highest Specificity rates, that are, in this case, the same as for the overall Accuracy: the Radial SVM (85.7%), followed by the QDA model (76.19%) and the Random Forest model (66.7%). 

On the other hand, if the aim of the project is to detect and predict bad creditors, then the models considered should be the ones having the highest Sensitivity rates: the Radial SVM model (100%), followed by the Random Forest model (75.61%) and the Logistic Regression or LDA models (both at 73.2%).

With this in mind, we could state that we do have indeed a clear winner and that is the Radial SVM model, since it has the highest Accuracy, Specificity and Sensitivity rates.
