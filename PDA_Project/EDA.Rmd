...
This finding is particularly interesting as it raises the following question: is the value of a categorical variable discriminatory for the outcome of the response variable? For instance, if we have an observation where "CO.APPLICANT" = 1, what is the value of "RESPONSE"? QUESTION: Est ce qu'on met ca ou ce n est pas clair? H: Je suis pas sûre que ca soit clair, tu veux dire quoi par la?


Let us now analyse the numerical variables. For those, we can look at the min and the max, which gives us a range. For instance, we see that in this bank the duration of the loan goes from 4 to 72 months. The larger the range, the higher the chance of having outliers. For example, the "AMOUNT" variable has a very big range: from 250 to 18'424.

H: Est-ce qu'on développe un peu plus pour les numerical variables? 
...
H: Alternative
We confirm that for most of them, the number of instances in each level of the variables are unbalanced.
--> Il n'y a pas que des 0 et 1.
...
```
--> age, amount and duration skewed on the right --> big difference between mean and median, to be checked
--> enlever density plot pour install_rate et num_credits (ce ne sont pas des valeurs continues)
--> num_dep into binary 
--> change as factor
...
We note that there are no discrimination for any categorical variable when considering the RESPONSE feature, meaning that we are not going to be able to define RESPONSE only by looking at those variables indivdually. H: --> Plus clair comme ça?
...
From the correlation matrix we see that only duration and amount are correlated. Moreover, they are positively correlated. It seems like there is no correlation between the other variables, meaning that they are independent, which is actually positive since it means that we will not have problems of multicollinearity. 
