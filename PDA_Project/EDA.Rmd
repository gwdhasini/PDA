...
## NUM_DEPENDENT a juste 2 valeurs, 1 ou 2. est ce qu'on la considère aussi comme categorical variable?
...
show_plot(cat.var) #PIA: je comprends pas trop comment on peut interpreter ce graph
...
for (i in (1:30)) {
  if (is.factor(GermanCredit[,i])) {
    ...
    GermanCredit %>% 
    select(TELEPHONE,RESPONSE) %>% 
    explore_all(target= RESPONSE)
  }
} 

## PIA ca marche juste quand on select 3 variables, whyyyyyyy?
GermanCredit %>% 
